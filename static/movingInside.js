...
draggables.forEach((draggable) => {
  ...
  draggable.addEventListener('drop', async (event) => {
    ...
    const targetFolder =
      targetItem.getAttribute('data-type') === 'folder' ? targetItem.getAttribute('href').replace('uploads/', '') : null
...
async function moveFileToFolder(file, fileName, folder) {
  try {
    ...
    console.log('currentDir', location.pathname)
    const uploadInFolder = await axios.post(
      '/move' + location.pathname.replace('/uploads', '') + folder.replace('/', ''),
      formData,
      {
        params: {
          fileName,
          currentDir: location.pathname.replace('/uploads', ''),
        },
      },
    )
